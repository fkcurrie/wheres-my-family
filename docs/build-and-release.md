...
The GitHub Actions workflow compiles the signed `.ipa` file and uploads it directly to App Store Connect. To ensure users are presented with clear release notes:
* The submission should include release note summaries.
* For automated submission scripts, pass the `--what-to-test` flag to describe the key updates.
* **Example:**
  `npx eas-cli submit --platform ios --profile production --what-to-test "• Added speed-adaptive telemetry rates\n• Implemented beautiful map zoom transitions"`
